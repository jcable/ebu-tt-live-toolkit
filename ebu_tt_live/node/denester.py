...
class DenesterNode(AbstractCombinedNode):
    ...
    def process_document(self, document, **kwargs):
        ...
        for div in divs:
            unnested_divs.extend(recurse(div,dataset))
        unnested_divs = combine_divs(unnested_divs)
        unnested_divs = check_p_regions(unnested_divs)
...
def combine_divs(divs):
    ...
    if len(divs) != 0:
        ...
        j = 0
        while i < len(divs):
            if div_attr(divs[i]) == div_attr(divs[i-1]):
...
def merge_attr(parent_attr, div_attributes):
    ...
    if parent_attr["end"] is not None and div_attributes["end"] is not None:
        ...
        merged_attributes["end"] = calculate_end_times(parent_attr, div_attributes, parent_attr["begin"])
...
def recurse( div, dataset, merged_attr={"styles": [], "begin": None, "end": None, "lang": None, "region": None }):
    merged_attr = merge_attr(merged_attr, div_attr(div))
    new_divs = []
    for c in div.orderedContent():
        if isinstance(c.value,div_type):
            if(div.region != c.value.region and c.value.region !=None and div.region !=None):
                continue
            new_divs.extend(recurse(c.value, dataset, merged_attr))
            ...
            for ic in c.value.orderedContent():
                if isinstance(ic.value,span_type):
                    new_spans.extend(recurse_span(ic.value,dataset))
            ...
            new_div = div_type(
                ...
                begin=process_timing_from_timedelta(merged_attr["begin"]) if merged_attr["begin"] is not None else merged_attr["begin"],
                end=process_timing_from_timedelta(merged_attr["end"]) if merged_attr["end"] is not None else merged_attr["end"],
...
def recurse_span(span, dataset, span_styles = []):
    ...
    for sc in span.orderedContent():
        if isinstance(sc.value,span_type):
            new_spans.extend(recurse_span(sc.value, dataset, span_styles))
        else:
            new_span = span_type(
                sc.value
            )
            ...
            if len(span_styles) != 0:
                new_span.style = compute_span_merged_styles(span_styles, dataset).id if len(span_styles) >1 else span_styles
            else:
                new_span.style = None
...
def compute_span_merged_styles(span_styles, dataset):
    ...
    new_style = style_type(
        id="".join(span_styles),
        backgroundColor=get_value_from_style(styles, "backgroundColor"),
        color=get_value_from_style(styles, "color"),
        fontFamily=get_value_from_style(styles, "fontFamily"),
        fontSize=calculate_font_size(styles),
        fontWeight=get_value_from_style(styles, "fontWeight"),
        lineHeight=get_value_from_style(styles, "lineHeight"),
        linePadding=get_value_from_style(styles, "linePadding"),
        padding=get_value_from_style(styles, "padding"),
        style=get_value_from_style(styles, "style"),
        textAlign=get_value_from_style(styles, "textAlign"),
        textDecoration=get_value_from_style(styles, "textDecoration")
    )
    new_style = create_new_style(new_style, dataset)
    return new_style
...
def calculate_font_size(styles):
    ...
    for style in styles:
        if font_size is None: # No existing font size
            ...
            font_size = calculate_percentage_font_size(font_size, style.fontSize)
